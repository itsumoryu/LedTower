...
int getReading(){
  int firstRead = analogRead(sensorPin);
  delay(1);
  int secondRead = analogRead(sensorPin);
  delay(1);
  int thirdRead = analogRead(sensorPin);
  int avgRead = (firstRead + secondRead + thirdRead)/3;
  int tenPercentDiff = avgRead * 1.10;
  while (abs(firstRead) > tenPercentDiff || abs(secondRead) > tenPercentDiff || abs(thirdRead) > tenPercentDiff){
    firstRead = analogRead(sensorPin);
    delay(1);
    secondRead = analogRead(sensorPin);
    delay(1);
    thirdRead = analogRead(sensorPin);
    avgRead = (firstRead + secondRead + thirdRead)/3;
    tenPercentDiff = avgRead * 1.10;
  }
  return avgRead;
}
